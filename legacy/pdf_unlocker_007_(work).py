...
def repair_pdf_with_gs(file_path, output_path):
    ...
    try:
        ...
        return False, f"Ghostscript錯誤: {e!s}"
...
def repair_and_unlock_pdf(file_path, output_path, password=None, available_packages=None):
    ...
    for method_name, method_func in methods:
        try:
            ...
            if method_name == "Ghostscript" or method_name == "簡單複製":
...
def unlock_pdfs():
    ...
    file_paths = filedialog.askopenfilenames(
        title="選擇要解鎖的PDF檔案", filetypes=[("PDF檔案", "*.pdf")]
    )
    ...
    for i, file_path in enumerate(file_paths, 1):
        try:
            ...
            success, message = repair_and_unlock_pdf(
                file_path, output_path, None, available_packages
            )
...
def unlock_pdfs_with_password():
    ...
    root.withdraw()
    file_paths = filedialog.askopenfilenames(
        title="選擇需要密碼解鎖的PDF檔案", filetypes=[("PDF檔案", "*.pdf")]
    )
    ...
    password_entry = tk.Entry(
        password_window, textvariable=password_var, show="*", width=35, font=("Arial", 11)
    )
    ...
    for i, file_path in enumerate(file_paths, 1):
        try:
            ...
            success, message = repair_and_unlock_pdf(
                file_path, output_path, password, available_packages
            )
...
def main():
    ...
    title_label = tk.Label(
        root, text="多引擎PDF修復工具", font=("Arial", 20, "bold"), fg="darkblue"
    )
...
適用於各種PDF問題：加密、損壞、格式錯誤等"""
    ...
    info_label = tk.Label(
        info_frame, text=info_text, font=("Arial", 11), justify=tk.LEFT, fg="darkgreen"
    )
    info_label.pack()
...
5. 直接關閉視窗也會正常退出程式"""
    ...
    bottom_label = tk.Label(
        bottom_frame, text=bottom_text, font=("Arial", 9), justify=tk.LEFT, fg="gray"
    )
    bottom_label.pack()
